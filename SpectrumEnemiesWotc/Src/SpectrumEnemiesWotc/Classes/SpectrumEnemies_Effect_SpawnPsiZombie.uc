class SpectrumEnemies_Effect_SpawnPsiZombie extends X2Effect_SpawnUnit;
...
function OnSpawnComplete(const out EffectAppliedData ApplyEffectParameters, StateObjectReference NewUnitRef, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	if (DeadUnitGameState.GetMyTemplateName() == 'AdvTrooperP3')
	{
		AbilityTemplateMgr = class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager();
		
		AbilityTemplate = AbilityTemplateMgr.FindAbilityTemplate('SpectrumEnemiesPsiExplode');
		AbilityRef = `TACTICALRULES.InitAbilityForUnit(AbilityTemplate, ZombieGameState, NewGameState, ApplyEffectParameters.ItemStateObjectRef);
	...
	NewEffectParams.EffectRef.SourceTemplateName = class'SpectrumEnemies_Ability_Psi'.default.ZombieLinkName;
...
defaultproperties
{
	...
	AnimationName="HL_GetUp_Reanimate"

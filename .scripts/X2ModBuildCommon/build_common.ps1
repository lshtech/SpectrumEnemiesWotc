...
class BuildProject {
	...
	[void]InvokeBuild() {
		try {
			...
			if ($this._HasScriptPackages()) {
				$this._PerformStep({ ($_)._RunMakeModName() }, "Compiling", "Compiled", "mod script packages")
			}
	...
	[void]_RunMakeModName() {
		#build the mod's scripts
		Write-Host "Attempting to compile mod scripts"
	...
	[void]_RunMakeModDirs() {
		# build the mod's scripts
		Write-Host "Attempting to compile mod scripts"
		foreach ($name in $this.modScriptPackages)
		{
			$scriptsMakeArguments = "make -nopause -mods $($name) $($this.stagingPath)"
			if ($this.debug -eq $true)
			{
				$scriptsMakeArguments = "$scriptsMakeArguments -debug"
			}
